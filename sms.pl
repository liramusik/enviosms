...
if ( defined($tty) and  defined($municipio) and  defined($tipo) and  defined($mensaje) ){
	if ($tipo eq 'm') {
		my $sql = "SELECT telefono FROM tachira WHERE id_municipio = 28 AND telefono like \"0416%\""; 
	} elsif ($tipo eq 'c') {
		my $sql = "SELECT telefono FROM tachira WHERE cedula = 16409503"; 
	} else {
		exit();
	}
	
	my $modem = new Device::Modem(log => 'File', port => $tty);
